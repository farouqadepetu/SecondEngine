...
void main()
{
    ...
    vec4 posH = mvp * inPos;
    vec4 posW = perObjectBuffer.model * inPos;
    vec4 normal = normalize(perObjectBuffer.transposeInverseModel * inNormal);
    vec4 tangent = normalize(perObjectBuffer.transposeInverseModel * inTangent);
