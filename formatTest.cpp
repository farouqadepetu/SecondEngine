...
uint32_t SEMax(uint32_t v1, uint32_t v2)
{
    if (v1 >= v2)
        return v1;
...
void ReadDDSFile(const char* filename, uint8_t** bitData, uint32_t* numBytes, DDS_HEADER* ddsHeader, DDS_HEADER_DXT10* ddsHeader10)
{
    FILE* file = nullptr;
    ...
    if (!file)
    {
        printf("Failed to open file %s. Exiting program.\n", filename);
    ...
    {
        printf("%s is not a DDS file. Exiting Progam.\n", filename);
        fclose(file);
        exit(4);
    }

    fread(ddsHeader, sizeof(DDS_HEADER), 1, file);
    printf("DDS Header size = %d\n", ddsHeader->size);
    if (ddsHeader->size != sizeof(DDS_HEADER))
    {
        printf("DDS Header file size != 124 bytes. Exiting Progam.");
        fclose(file);
        exit(4);
    }

    printf("DDS Pixel Format file size = %d\n", ddsHeader->ddspf.size);
    if (ddsHeader->ddspf.size != sizeof(DDS_PIXELFORMAT))
    {
        printf("DDS Pixel Format file size != 32 bytes. Exiting Progam.");
        fclose(file);
        exit(4);
    }
...
void FreeBitData(uint8_t* bitData)
{
    free(bitData);
}
...
int RetrieveTextureInfo(const DDS_HEADER* const ddsHeader, const DDS_HEADER_DXT10* const ddsHeader10, uint8_t* bitData, const uint32_t numBytes,
    SETextureInfo* textureInfo)
{
    ...
    if (ddsHeader->ddspf.flags & DDS_FOURCC && ddsHeader->ddspf.fourCC == DX10)
    {
        ...
        textureInfo->format = ddsHeader10->dxgiFormat;
    ...
    {
        textureInfo->format = GetDXGIFormat(ddsHeader->ddspf);
        ...
        if (BitsPerPixel(textureInfo->format) == 0)
        {
            return SE_NOT_SUPPORTED;
        }
    }
    ...
    printf("\nWidth = %d\nHeight = %d\nDepth = %d\nMipCount = %d\nArraySize = %d\nDimension = %d\nFormat = %d\nisCubeMap = %d\n",
        textureInfo->width, textureInfo->height, textureInfo->depth, textureInfo->mipCount, 
        textureInfo->arraySize, textureInfo->resDim, textureInfo->format, textureInfo->isCubeMap);
    ...
    for (uint32_t i = 0; i < textureInfo->arraySize; ++i)
    {
        ...
        for (uint32_t j = 0; j < textureInfo->mipCount; ++j)
        {
            ...
            printf("\nArraySize = %d\nMipNum = %d\nWidth = %d\nHeight = %d\nDepth = %d\nRowBytes = %d\nNumRows = %d\nNumBytes = %d\n",
                i, j, width, height, depth, rowBytes, numRows, bytesCount);
...
int main()
{
    ...
    ReadDDSFile("statue.dds", &bitData, &numBytes, &ddsHeader, &ddsHeader10);

    SETextureInfo texInfo{};
